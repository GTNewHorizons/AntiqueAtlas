...
public class MarkerApiImpl implements MarkerAPI {
    ...
    public void setTexture(String markerType, ResourceLocation texture) {
        MarkerTextureMap.instance()
            .setTexture(markerType, texture);
    }

    @Override
    public void putMarker(World world, boolean visibleAhead, int atlasID, String markerType, String label, int x,
        int z) {
    ...
    private void doPutMarker(World world, boolean visibleAhead, int atlasID, String markerType, String label, int x,
        int z) {
        if (world.isRemote) {
            if (atlasID == GLOBAL) {
            ...
            if (atlasID == GLOBAL) {
                ...
                Marker marker = data
                    .createAndSaveMarker(markerType, label, world.provider.dimensionId, x, z, visibleAhead);
    ...
    private void doDeleteMarker(World world, int atlasID, int markerID) {
        DeleteMarkerPacket packet = atlasID == GLOBAL ? new DeleteMarkerPacket(markerID)
        ...
        if (world.isRemote) {
            ...
            MarkersData data = atlasID == GLOBAL ? AntiqueAtlasMod.globalMarkersData.getData()
