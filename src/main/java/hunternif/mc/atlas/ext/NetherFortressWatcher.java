...
public class NetherFortressWatcher {
    ...
    private static final String ROOFED4 = "NeSC"; // ? Roofed? Covers most of the area of the Fortress. Solid wall down
                                                  // to the ground?

    // Crossings:
    private static final String BRIDGE_GATE = "NeRC"; // That room with no roof with gates facing each direction. One
                                                      // thick solid column going down to the ground. -done!
    private static final String ROOFED_CROSS = "NeSCSC"; // Roofed corridor?
    private static final String BRIDGE_CROSS = "NeBCr"; // A crossing of open bridges. No roof, no column. Takes up
                                                        // 19x19 area because of the beginnings of bridges starting off
                                                        // in different directions. - done!
    ...
    private static final String ENTRANCE = "NeCE"; // "Entrance", a very large room with an iron-barred gate. Contains a
                                                   // well of lava in the center.
    private static final String WART_STAIRS = "NeCSR"; // Room with the Nether Wart and a wide staircase leading to an
                                                       // open roof with a fence railing.
    ...
    public void visitAllUnvisitedFortresses(World world) {
        MapGenStructureData data = (MapGenStructureData) world.perWorldStorage
            .loadData(MapGenStructureData.class, "Fortress");
        if (data == null) return;
    ...
    private static boolean noTileAt(World world, int chunkX, int chunkZ) {
        return AntiqueAtlasMod.extBiomeData.getData()
            .getBiomeIdAt(world.provider.dimensionId, chunkX, chunkZ) == -1;
    }
}
