...
public class ExportImageUtil {
    ...
    public static void exportPngImage(DimensionData biomeData, DimensionMarkersData globalMarkers,
        ...
        float updateUnitsTotal = biomeData.getSeenChunks()
            .size();
        if (showMarkers) {
            updateUnitsTotal += globalMarkers.getAllMarkers()
                .size();
            if (localMarkers != null) {
                updateUnitsTotal += localMarkers.getAllMarkers()
                    .size();
            }
        }
        int updateUnits = 0;
        ...
        try {
            InputStream is = Minecraft.getMinecraft()
                .getResourceManager()
                .getResource(Textures.EXPORTED_BG)
                .getInputStream();
            bg = ImageIO.read(is);
            is.close();
        ...
        // Top left corner:
        graphics
            .drawImage(bg, 0, 0, BG_TILE_SIZE * scale, BG_TILE_SIZE * scale, 0, 0, BG_TILE_SIZE, BG_TILE_SIZE, null);
        updateUnits++;
        // Topmost row:
        for (int x = 1; x < bgTilesX; x++) {
            graphics.drawImage(
                bg,
                x * BG_TILE_SIZE * scale,
                0,
                (x + 1) * BG_TILE_SIZE * scale,
                BG_TILE_SIZE * scale,
                BG_TILE_SIZE,
                0,
                BG_TILE_SIZE * 2,
        ...
        for (int y = 1; y < bgTilesY; y++) {
            graphics.drawImage(
                bg,
                0,
                y * BG_TILE_SIZE * scale,
                BG_TILE_SIZE * scale,
                (y + 1) * BG_TILE_SIZE * scale,
                0,
                BG_TILE_SIZE,
                BG_TILE_SIZE,
                BG_TILE_SIZE * 2,
        ...
        while (iter.hasNext()) {
            ...
            for (SubTile subtile : subtiles) {
                ...
                // Load tile texture
                ResourceLocation texture = BiomeTextureMap.instance()
        ...
        if (showMarkers) {
            ...
            for (int x = biomeData.getScope().minX / MarkersData.CHUNK_STEP; x
                <= biomeData.getScope().maxX / MarkersData.CHUNK_STEP; x++) {
                for (int z = biomeData.getScope().minY / MarkersData.CHUNK_STEP; z
                    <= biomeData.getScope().maxY / MarkersData.CHUNK_STEP; z++) {
                    ...
                    for (Marker marker : markers) {
                        ...
                        // Load marker texture
                        ResourceLocation texture = MarkerTextureMap.instance()
