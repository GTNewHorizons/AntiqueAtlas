...
public class DimensionData implements ITileStorage {
    ...
    public void setTile(int x, int y, Tile tile) {
        ShortVec2 groupPos = getKey().set(
            (int) Math.floor(x / (float) TileGroup.CHUNK_STEP),
    ...
    public void readFromNBT(NBTTagList me) {
        ...
        for (int d = 0; d < me.tagCount(); d++) {
            ...
            ShortVec2 key = new ShortVec2(
                tg.getScope().minX / TileGroup.CHUNK_STEP,
                tg.getScope().minY / TileGroup.CHUNK_STEP);
    ...
    public boolean equals(Object obj) {
        ...
        for (ShortVec2 entry : tileGroups.keySet()) {
            if (!this.tileGroups.get(entry)
                .equals(other.tileGroups.get(entry))) return false;
        }
        return true;
    }
}
