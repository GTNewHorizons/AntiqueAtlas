...
public class TileRenderIterator implements Iterator<SubTileQuartet> {
    ...
    private final SubTile _d = new SubTile(Part.BOTTOM_RIGHT), _e = new SubTile(Part.BOTTOM_LEFT),
        _h = new SubTile(Part.TOP_RIGHT), _i = new SubTile(Part.TOP_LEFT);
    ...
    public boolean hasNext() {
        return chunkX >= scope.minX && chunkX <= scope.maxX + 1 && chunkY >= scope.minY && chunkY <= scope.maxY + 1;
    }
    ...
    private static boolean shouldStitchTo(Tile tile, Tile to) {
        ...
        TextureSet set = BiomeTextureMap.instance()
            .getTextureSet(tile);
        TextureSet toSet = BiomeTextureMap.instance()
            .getTextureSet(to);
