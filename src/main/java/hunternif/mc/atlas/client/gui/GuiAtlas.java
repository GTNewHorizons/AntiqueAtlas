...
public class GuiAtlas extends GuiComponent {
    ...
    public GuiAtlas() {
        ...
        btnShowMarkers = new GuiBookmarkButton(
            3,
            Textures.ICON_HIDE_MARKERS,
            I18n.format("gui.antiqueatlas.hideMarkers"));
        ...
        if (!state.is(NORMAL) && !state.is(HIDING_MARKERS)) {
            if (state.is(PLACING_MARKER) // If clicked on the map, place marker:
                ...
                addChildBehind(markerFinalizer, blinkingIcon)
                    .setRelativeCoords(mouseX - getGuiX() - MARKER_SIZE / 2, mouseY - getGuiY() - MARKER_SIZE / 2);
                ...
                && isMouseOverMap
                && mouseState == 0) {
                    AtlasAPI.markers.deleteMarker(player.worldObj, stack.getItemDamage(), toDelete.getId());
                }
            state.switchTo(NORMAL);
    ...
    private void exportImage(ItemStack stack) {
        ...
        if (file != null) {
            try {
                ...
                ExportImageUtil
                    .exportPngImage(biomeData, globalMarkersData, localMarkersData, file, progressBar, showMarkers);
