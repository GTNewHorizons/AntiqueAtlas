...
public class TestNBT {
    ...
    public void writeToNBTv2(AtlasData atlasdata, NBTTagCompound compound) {
        ...
        int dimension = 0;
        int key;
        Iterator<Integer> dimensionEntryKey = atlasdata.getVisitedDimensions()
            .iterator();
        for (dimension = 0; dimension < atlasdata.getVisitedDimensions()
            .size(); dimension++) {
            NBTTagCompound dimTag = new NBTTagCompound();
            key = dimensionEntryKey.next()
                .intValue();
